...
import { parseJsonBody, expectObject, expectEmail, expectPassword } from "@/lib/server/validation";
...
export async function POST(request: Request) {
  return withErrorHandling(request, async (requestId) => {
    ...
    const throttle = checkThrottle(tenantId, "auth:login", actorKey(request, email));
    if (throttle > 0) {
      return jsonError(requestId, 429, "THROTTLED", `Too many failed attempts. Retry in ${Math.ceil(throttle / 1000)}s`);
    }

    try {
      const token = await loginWithPassword({ tenantId, email, password });
      registerSuccess(tenantId, "auth:login", actorKey(request, email));
      return jsonOk(requestId, token);
    } catch (error) {
      registerFailure(tenantId, "auth:login", actorKey(request, email));
      throw error;
    }
  });
}